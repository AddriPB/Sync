...
export function SyncApp() {
  ...
  useEffect(() => {
    ...
    void refreshEvents(session.userId);
  }, [session]);
  ...
  async function refreshEvents(userId = session?.userId) {
    if (!userId) {
      return;
    }
    const [nextEvents, nextSources] = await Promise.all([
      getEvents(userId),
      ...
      <section className="phone-frame">
        <header className="topbar">
          <div>
            ...
            <h1>{view === "planning" ? "Agenda" : formatMonthTitle(parseISO(selectedDate))}</h1>
          </div>
          <div className="topbar-actions">
            <button className="icon-button" onClick={() => shift(-1)} aria-label="Précédent">
              <ChevronLeft size={18} />
            </button>
            <button className="icon-button" onClick={() => shift(1)} aria-label="Suivant">
    ...
    <main className="shell">
      <section className="auth-card">
        <div className="auth-copy">
          ...
          <p>
            Mobile, sombre et rapide. Crée ton identifiant puis organise tes événements sans e-mail.
          </p>
        </div>
  ...
  return (
    <section className="planning-view">
      {grouped.map((group) => (
        <article
          key={group.date}
          className={selectedDate === group.date ? "planning-day planning-day-active" : "planning-day"}
          onClick={() => onSelectDate(group.date)}
        >
          <header className="planning-day-header">
            <div>
              <p>{formatDayLabel(group.date)}</p>
              <span>{group.events.length} événement{group.events.length > 1 ? "s" : ""}</span>
            </div>
          </header>
  ...
  return (
    <section className="week-view">
      <div className="week-strip">
        {days.map((day) => {
          ...
          return (
            <button
              key={dateKey}
              className={selectedDate === dateKey ? "week-pill week-pill-active" : "week-pill"}
      ...
      <div className="week-grid">
        {days.map((day) => {
          const dateKey = format(day, "yyyy-MM-dd");
          const dayEvents = events.filter((event) => spansOnDay(event, day)).slice(0, 4);
          return (
            <article
              key={dateKey}
              className={selectedDate === dateKey ? "week-column week-column-active" : "week-column"}
              onClick={() => onSelectDate(dateKey)}
            >
            ...
            <button
              ...
              className={
                selectedDate === dateKey
                  ? "month-cell month-cell-selected"
                  : !isSameMonth(day, parseISO(selectedDate))
                    ? "month-cell month-cell-muted"
                    : "month-cell"
              }
              onClick={() => onSelectDate(dateKey)}
            >
    ...
    <div className="overlay">
      <section className="sheet">
        ...
        <div className="sheet-fields">
          ...
          <label className="field">
            ...
            <input
              ...
              onChange={(inputEvent) => patch({ location: inputEvent.target.value })}
            ...
            <div className="suggestions-row">
              {suggestions.map((suggestion) => (
                <button
                  ...
                  onClick={() => patch({ location: suggestion.label })}
                >
                  {suggestion.label}
                </button>
              ))}
          ...
          {readonlyExternal ? (
            <p className="hint-text">Cet événement provient d'une source externe en lecture seule.</p>
          ) : null}
        </div>
        ...
        <footer className="sheet-footer">
          ...
          <button
            className="primary-button"
            disabled={!canSubmit || readonlyExternal}
            onClick={() => {
              void onSave(values);
            }}
          >
...
function TimeSelect({
  ...
  const options = [""].concat(
    Array.from({ length: 24 }, (_, hour) =>
      MINUTE_OPTIONS.map((minute) => `${String(hour).padStart(2, "0")}:${minute}`)
    ).flat()
  );
        ...
        <div className="sheet-fields">
          ...
          {connections.map((connection) => (
            <div key={connection.source} className="source-row">
              ...
              <button
                className={connection.enabled ? "switch switch-active" : "switch"}
                onClick={() => onToggleSource(connection.source, !connection.enabled)}
              >
                <span />
              </button>
            </div>
